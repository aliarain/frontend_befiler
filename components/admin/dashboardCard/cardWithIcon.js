...
const CardWithIcon = ({ iconCards }) => {
    ...
    return (
        <div>
            <div className={`bg-white p-3 h-40 2xl:h-36 lg:w-full rounded shadow-sm  my-2 lg:my-0 ${iconCards?.bgClr}`}>
                <div className='h-18 flex justify-between'>
                    ...
                    <div className='text-right'>
                        <p className='text-[16px] mb-0'>{iconCards?.tittle}</p>
                        <p className='text-xl text-gray-700'>{iconCards?.number}</p>
                    </div>
                </div>
                <div className="flex items-end border-t-2 py-2">
                    <div className='flex'>

...
const Tickets = () => {
    ...
    const handleImage = async (e) => {
        ...
        const d = await AWSFileUploadAPI(data)
        if(d.status === false)
            return null
        return d.url
    }
    ...
    const onFinish = async (values) => {
        ...
        values.files = await Promise.all(values.files.map(async (file) => {
            return await handleImage(file.originFileObj)
        }))
                        ...
                        columns={[
                            ...
                            },
                            {
                                ...
                                formatter: (cell) => cell === 'pending' ?
                                    <div
                                        className={'px-2 py-1 rounded-2xl border-2 border-twSecondary-shade700 text-center text-twSecondary-shade700'}>Pending</div> :
                                    cell === 'closed' ?
                                        <div
                                            className={'px-2 py-1 rounded-2xl border-2 border-red-500 text-center text-red-500'}>Closed</div> :
                                        <div
                                            className={'px-2 py-1 rounded-2xl border-2 border-green-500 text-center text-green-500'}>Open</div>
                            }
                        ]}
                        data={tickets}
                        shadow={false}
                        onEdit={(data) => {
                            Router.push(`/user/support-ticket/${data._id}`)
                        }}
                        action={(
                            <Button onClick={() => {

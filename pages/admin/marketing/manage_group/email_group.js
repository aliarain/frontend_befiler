...
const UserEmail = () => {
    ...
    const [smsGroups, getSMSGroups, {loading, error}] = useFetch(fetchMarketingGroups, {type: 'email'});
    let action = (
        <div className="flex">
            <Button
                className="mr-2"
    ...
    const columns = [
        ...
        },
        {
            ...
            formatter: (cell, row) =>
                <span
                    className={'px-2 py-1 rounded bg-off_purple text-white cursor-pointer'}
    ...
    return (
        <div>
            <PageTitle title="User Email Groups"/>
            ...
            <Modal title={`Email Group`} visible={isModalVisible} onCancel={() => setIsModalVisible(false)}
                   destroyOnClose
                   footer={null} width={569}>
                <Form
                    form={form}
                    onFinish={(values) => {
                        return userAction(postMarketingGroup, {...values, type: "email"}, () => {
                            getSMSGroups();
                            setIsModalVisible(false);
                        })
                    }}
                ...
                >
                    ...
                    <Button>{isEdit? 'Edit Email Group':'Add Email Group'} </Button>
                </Form>
            </Modal>
        </div>
    );
